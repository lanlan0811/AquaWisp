...
export function createDesktopMarkup(state: DesktopViewState): string {
  ...
  const browserBody = state.browserVisible
    ? `<section class="right-body browser-body" data-right-body="browser" hidden><form class="browser-address" data-browser-address-form><button type="button" data-browser-command="back" aria-label="后退">${icon("back")}</button><button type="button" data-browser-command="forward" aria-label="前进">${icon("forward")}</button><button type="button" data-browser-command="reload" aria-label="刷新">${icon("reload")}</button><input data-browser-address aria-label="浏览器地址" value="about:blank"><button type="submit" aria-label="打开地址">${icon("go")}</button><button type="button" data-browser-command="newTab" aria-label="新建标签页">${icon("plus")}</button><button type="button" data-browser-command="close" aria-label="关闭标签页">${icon("close")}</button></form><div class="browser-webviews" data-browser-webviews><webview data-browser-webview src="about:blank"></webview></div><p class="browser-status" data-browser-status>正在连接浏览器宿主…</p></section>`
    : "";
...
export const desktopRendererScript = `(() => {
  ...
  const browserAddressForm = document.querySelector("[data-browser-address-form]");
  const browserAddress = document.querySelector("[data-browser-address]");
  const browserWebviews = document.querySelector("[data-browser-webviews]");
  ...
  document.querySelectorAll("[data-right-tab]").forEach((button) => button.addEventListener("click", () => {
    ...
    if (name === "browser") void refreshBrowserState();
  }));
  const renderBrowserState = (state) => {
    if (!state || !Array.isArray(state.tabs)) return;
    browserState = state;
    const tabIds = new Set(state.tabs.map((tab) => String(tab.id)));
    const unassigned = browserWebviews?.querySelector("[data-browser-webview]:not([data-browser-tab-id])");
    if (unassigned instanceof HTMLElement && state.tabs.length === 1) unassigned.dataset.browserTabId = String(state.tabs[0].id);
    browserWebviews?.querySelectorAll("[data-browser-webview]").forEach((webview) => {
      const tabId = webview.dataset.browserTabId;
      if (tabId && !tabIds.has(tabId)) webview.remove();
      else webview.hidden = tabId !== String(state.activeTabId);
    });
    const active = state.tabs.find((tab) => String(tab.id) === String(state.activeTabId));
    if (browserAddress instanceof HTMLInputElement && active) browserAddress.value = active.url;
    if (browserStatus) browserStatus.textContent = active ? String(state.tabs.length) + " 个标签页 · " + active.url : "没有可用的浏览器标签页";
  };
  const refreshBrowserState = async () => {
    try { renderBrowserState(await api.browser.state()); }
    catch (error) { if (browserStatus) browserStatus.textContent = error instanceof Error ? error.message : "浏览器状态读取失败"; }
  };
  const executeBrowserCommand = async (command) => {
    const state = browserState ?? await api.browser.state();
    const tabId = state.activeTabId ?? state.tabs[0]?.id ?? (command.kind === "newTab" ? "browser-root" : undefined);
    if (!tabId) throw new Error("没有可用的浏览器标签页");
    browserRequestCounter += 1;
    const result = await api.browser.execute({ requestId: "renderer-browser-" + String(browserRequestCounter), backendGeneration: state.backendGeneration, tabId: String(tabId), command });
    await refreshBrowserState();
    return result;
  };
  browserAddressForm?.addEventListener("submit", (event) => {
    event.preventDefault();
    if (!(browserAddress instanceof HTMLInputElement)) return;
    void executeBrowserCommand({ kind: "navigate", url: browserAddress.value.trim() }).catch((error) => { if (browserStatus) browserStatus.textContent = error instanceof Error ? error.message : "导航失败"; });
  });
  document.querySelectorAll("[data-browser-command]").forEach((button) => button.addEventListener("click", () => {
    const kind = button.dataset.browserCommand;
    if (!kind) return;
    void executeBrowserCommand({ kind }).catch((error) => { if (browserStatus) browserStatus.textContent = error instanceof Error ? error.message : "浏览器命令失败"; });
  }));
  api.browser.onStateChanged(renderBrowserState);
  api.browser.onCreateTab((message) => {
    if (!(browserWebviews instanceof HTMLElement)) return;
    const webview = document.createElement("webview");
    webview.dataset.browserWebview = "";
    webview.dataset.browserCreateRequest = message.requestId;
    webview.src = message.url;
    browserWebviews.append(webview);
  });
  api.browser.onTabRegistered((message) => {
    const webview = browserWebviews?.querySelector('[data-browser-create-request="' + CSS.escape(message.requestId) + '"]');
    if (webview instanceof HTMLElement) {
      webview.dataset.browserTabId = message.tabId;
      delete webview.dataset.browserCreateRequest;
    }
  });
  api.browser.onActivateTab((message) => {
    browserWebviews?.querySelectorAll("[data-browser-webview]").forEach((webview) => { webview.hidden = webview.dataset.browserTabId !== message.tabId; });
  });
