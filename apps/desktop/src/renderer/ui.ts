...
export function createDesktopMarkup(state: DesktopViewState): string {
  ...
  return `<main class="app-shell"><aside class="sidebar"><button class="new-session">${icon("plus")}新建会话</button><nav><button class="nav active">${icon("chat")}会话</button><button class="nav">${icon("library")}知识库</button></nav><footer>${label} · ${runtimeLabel}</footer></aside><section class="conversation"><header>${state.workspaceName}<span>${state.modelName}</span></header><article class="assistant-message"><b>沧</b><p>你好，我是沧渡。你可以让我采集资料、整理知识库或生成文档。</p></article><section class="input-card"><textarea placeholder="帮你采集资料、整理知识库、生成文档报告……"></textarea><button class="send" aria-label="${state.running ? "停止" : "发送"}">${icon(state.running ? "stop" : "send")}</button></section></section></main>`;
}
export const desktopStyles = `:root{--brand:#0e7490;--send:#34b3a0;--side:#f7f7f8;--border:#e8e8ea;--text:#1f2933;font-family:system-ui,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif}body{margin:0;color:var(--text)}.app-shell{display:flex;min-height:100vh}.sidebar{width:220px;background:var(--side);border-right:1px solid var(--border);padding:16px;display:flex;flex-direction:column}.new-session,.send{border:0;border-radius:6px;color:#fff}.new-session{background:var(--brand);padding:8px 16px;font-weight:600}.nav{margin-top:16px}.nav button{display:flex;gap:8px;border:0;background:transparent;padding:10px}.nav .active{border-left:3px solid var(--brand);color:var(--brand)}footer{margin-top:auto;font-size:12px}.conversation{flex:1}.conversation header{height:44px;border-bottom:1px solid var(--border);padding:10px 24px;display:flex;justify-content:space-between}.assistant-message{display:flex;gap:8px;max-width:860px;margin:24px auto}.assistant-message p{padding:12px 16px;border:1px solid var(--border);border-radius:4px 16px 16px}.input-card{width:min(800px,calc(100% - 48px));margin:40vh auto 24px;border:1px solid var(--border);border-radius:12px;padding:12px;display:flex}.input-card:focus-within{border-color:var(--brand);box-shadow:0 2px 16px rgba(14,116,144,.12)}textarea{flex:1;border:0;min-height:80px;font:inherit}.send{width:36px;height:36px;background:var(--send)}svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.8}`;
function icon(name: "plus" | "chat" | "library" | "send" | "stop"): string {
  const paths = {
