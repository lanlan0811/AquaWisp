...
export const desktopRendererScript = `(() => {
  ...
  const inputModelButton = document.querySelector("[data-input-model]");
  ...
  const submitConversation = async () => {
    ...
    setRunning(true);
    try {
      const result = await api.conversation.start({ sessionId, userInput, mode: sessionMode });
  ...
  inputModelButton?.addEventListener("click", () => showView("settings"));
  ...
  form?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      for (const modelLabel of document.querySelectorAll("[data-model-label]")) modelLabel.textContent = model.selectedOptions[0]?.textContent ?? saved.modelId;
