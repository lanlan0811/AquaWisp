...
| `apps/desktop/src/desktop-config.data.json`       | Electron main/preload      | Window, runtime, execution-mode labels/confirmation policy, settings defaults, environment, secret/import limits and IPC channels | zod schema, preload drift check and packaged-process tests           |
