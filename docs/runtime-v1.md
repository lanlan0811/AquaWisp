...
`runtime.run.start` selects a catalog-validated OpenAI-compatible model and an explicit execution mode, prepares hash-verified system prompts and persisted session context, then streams committed `RunEvent` envelopes before returning the terminal Run. The model adapter declares the configured tool registry in native Chat Completions or Responses shape, assembles fragmented function arguments by call ID, rejects multiple calls in one reason cycle, and maps model-safe names back to canonical tool IDs. Each verified observation is labeled untrusted and included in the next cycle. Interrupted text streams may emit `model.stream.recovery`; a partially emitted tool call is never resumed because replay could duplicate a side effect.

The production tool runtime validates inputs against the registry JSON Schemas before policy evaluation and again before dispatch. The desktop-selected mode drives the fixed-order evaluator. File, search, terminal, web-fetch, and knowledge operations use their existing bounded adapters; aggregate arguments and observations have separate configured byte ceilings so a valid tool result cannot overflow the desktop JSONL envelope. `runtime.run.cancel` is handled concurrently and propagates into model, web, approval, and terminal waits. `runtime.approval.resolve` accepts only the exact pending IDs, and resolution persists before authorization. Only one Run may be active in this v1 host.
